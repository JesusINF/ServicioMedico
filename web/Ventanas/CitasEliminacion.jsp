...
<html lang="es_mx">
    ...
    <body>
        <div class="Form inicio-sesion">
            <h1>Eliminación Citas</h1>
            <div class="<c:out value="${Actualiza}"/>">
                <form class="login-form" action="MenuCitas?accion=BuscaElimina"method="POST" autocomplete="off" accept-charset="utf-8">
            ...
            <div class="<c:out value="${Busca}"/>">
                <form class="login-form" action="MenuCitas?accion=Elimina" method="POST" autocomplete="off" accept-charset="utf-8">
                    <input list="medico" name="medico" class="Block login-username" placeholder="Busca Medico" value="<c:out value="${DatoMedico}"/>" required >
                    <datalist id="medico">
                        <c:forEach var="Emp" items="${Lista1}">
                            <option value="<c:out value="${Emp}"/>">
                            </c:forEach>
                    </datalist>
                    <input list="paciente" name="paciente" class="Block login-username" placeholder="Busca Paciente" value="<c:out value="${DatoPaciente}"/>" required>
                    <datalist id="paciente">
                        <c:forEach var="Emp" items="${Lista2}">
                            <option value="<c:out value="${Emp}"/>">
                            </c:forEach>
                    </datalist>
                    <input type="datetime-local" id="inicio" name="inicio" class="Block login-username" required value="<c:out value="${Emp.inicio}"/>" >
                    <label for="inicio">Fecha de inicio de consulta estimada</label>
                    <input type="datetime-local" id="fin" name="fin" class="Block login-username" required value="<c:out value="${Emp.fin}"/>" >
        ...
        <div id="popup1" class="<c:out value="${pop1}"/> overlay">
            <div class="popup">
                ...
                <a class="close" href="MenuCitas?accion=Eliminar">&times;</a>
                <div class="content">
